...
def plot_graph_and_trajectories(sequence, actual_future, predicted_future, ax):
    ...
    x_actual, y_actual = zip(*[pos[:2] for pos in actual_future])
    ax.scatter(x_actual, y_actual, c='green', s=30, label='Actual future')

    # Plot predicted future trajectory
    x_pred, y_pred = zip(*[pos[:2] for pos in predicted_future])
...
def make_predictions(model, dataset, device, num_samples=9):
    ...
    with torch.no_grad():
        for idx in sampled_sequences:
            ...
            past = past.unsqueeze(0).to(device)  # Add batch dimension

            continuous_pred, binary_pred = model(past)

            # Combine predictions
            predictions = torch.cat((continuous_pred, binary_pred), dim=-1)

            all_predictions.append(predictions.squeeze().cpu().numpy())
            all_actual.append(future.numpy())
