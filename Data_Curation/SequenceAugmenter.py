...
class SequenceAugmenter:
    ...
    def augment(self):
        augmented_sequences = [deepcopy(self.sequence)]  # Include the original sequence
        #augmented_sequences = []

        # Generate equally spaced rotation angles
        rotation_angles = np.linspace(0, 360, config.NUM_ROTATIONS, endpoint=False)
        
        for angle in rotation_angles:
            rotated_sequence = self.rotate(angle)
            augmented_sequences.append(deepcopy(rotated_sequence))
            
            # Apply mirroring after each rotation
            for axis in config.MIRRORS:
                self.sequence = deepcopy(rotated_sequence)  # Reset the sequence to the rotated version
